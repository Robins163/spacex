...
export const Page = () => {
    ...
    const previousYearOption = (totalYears = 0) => {
        ...
        let currentYear = moment().subtract(initialYearDiff, 'years').format('YYYY')
    ...
    return (
        <Container>
            <div className={'card p-2 my-3 mx-1'}>
                <Row className={'gx-2'}>
                    <Col>
                        <input onChange={handleChange} type="text" className="form-control" placeholder="Search Rocket"
                               name="rocket_name" value={rocketName}/>
                    </Col>
                    <Col>
                        <select className="form-select" name="launch_status" value={launchStatus}
                    ...
                    <Col>
                        <select className="form-select" value={timeRange} onChange={handleChange}
                    ...
                    <Col>
                        <select className="form-select" name='upcoming' value={upcoming}
                        onChange={handleChange}>
                                ...
                                rockets.map((rocket, index) => {
                                    return (
                                        <div key={index} className="col-3 col-md-3 card p-3">
                                            <img className={'rounded img-fluid card-img-top'}
                                                 src={rocket?.links?.mission_patch_small ? rocket?.links?.mission_patch_small : 'https://via.placeholder.com/292x346'} alt={rocket.mission_name}/>
                                            <div className={'mt-3'}>
                                                <h5 className={'card-title '}>
                                                    Mission Name: {rocket.mission_name}
                                                </h5>
                                                <div className="card-body">
                                                    <div className="card-text text-start"> Rocket
                                                        Name: {rocket.rocket.rocket_name}</div>
                                                    <div
                                                        className="text-start"> Date: {moment(rocket.launch_date_local).format('DD MMM YYYY')}</div>
                                                    <div
                                                        className="text-start"> Time: {moment(rocket.launch_date_local).format('HH:mm')}</div>
                                                    <div className="card-text">
                                                        <div
                                                            className={"alert " + (rocket.launch_success ? " alert-success" : " alert-danger")}
                                                            role="alert">
                                                            {rocket.launch_success ? 'Mission Success' : 'Mission Failed'}
                                                        </div>
                                                    </div>
                                                    <div className="card-text">
                                                        <div
                                                            className={"alert " + (rocket.upcoming ? " alert-success" : " alert-danger")}
                                                            role="alert">
